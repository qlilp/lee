...
【自动刷新缓存说明】
- 脚本会在同目录下生成 aliyunpan_token_cache.json 缓存文件
- 每次成功刷新 token 后自动将新 refresh_token 写入缓存
- 下次运行优先读取缓存中的 token，缓存失效时回退到环境变量中的初始 token
- 只需首次配置环境变量，后续 token 自动续期，无需手动更新
"""
...
# ==================== 缓存配置 ====================
# 缓存文件路径（与脚本同目录）
CACHE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "aliyunpan_token_cache.json")


def load_token_cache() -> Dict[str, str]:
    """读取本地 token 缓存，返回 {账号名: refresh_token}"""
    ...
    try:
        ...
        logger.warning(f"读取缓存文件失败: {e}，将使用环境变量中的 token")
    return {}
...
def main():
    ...
    if token_cache:
        logger.info(f"📦 已加载本地缓存，共 {len(token_cache)} 个账号的 token")
    ...
    for idx, acc_str in enumerate(account_list):
        ...
        # 按 # 分隔 token 和 备注
        parts = acc_str.split('#')
        env_token = parts[0].strip()
        name = parts[1].strip() if len(parts) > 1 else f"账号{idx+1}"

        if not env_token and name not in token_cache:
            logger.warning(f"账号 {name} 缺少 refresh_token，且缓存中也没有，跳过")
            continue

        # 优先使用缓存中的 token，缓存没有则用环境变量中的
        if name in token_cache:
            use_token = token_cache[name]
            logger.info(f"[{name}] 使用缓存中的 refresh_token")
        else:
            use_token = env_token
