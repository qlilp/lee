...
async def exchange(session, phone, title, aid, jsexec, ckvalue):
    try:
        ...
        get_url_js = await loop.run_in_executor(None,  lambda: jsexec.call("getUrl",  "POST", "https://wapact.189.cn:9001/gateway/stand/detailNew/exchange")) 
...
async def login_request(ss_session, url, payload):
    try:
        ...
        if response.status_code  == 412:
            ...
            loop = asyncio.get_event_loop() 
            ck_value_js = await loop.run_in_executor(None,  lambda: jsexec.call("getck")) 
            ...
            new_url_js = await loop.run_in_executor(None,  lambda: jsexec.call("getUrl",  "POST", url))
...
if __name__ == "__main__":
    ...
    loop = asyncio.get_event_loop() 
    loop.run_until_complete(main(api_time_diff  if api_time_diff > 0 else 0, is_true_flag, h))
