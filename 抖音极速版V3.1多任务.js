 ...
 * 项目名称：抖音极速版
 * 项目更新：2023-08-30
 * 项目抓包：抓api*-normal-c-lq.amemv.com下的宝箱url#cookie#x-argus#x-ladon#user-agent填入变量
...
let host = 'api5-normal-c-lq.amemv.com';
...
async function start() {
    ...
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.dailyread(2 * 1000));
        await $.wait(1000);
    }
    await Promise.all(taskall);
    DoubleLog(`\n================ 执行果树农场赚金 ================`)
...
class UserInfo {
    constructor(str) {
        this.index = ++userIdx;
        this.ck = str.split('#');
        this.xvcbdturingsdkversion = '3.6.2.cn';
        this.passportsdkversion = '203135';
        this.iid = this.ck[0].match(/iid=([\w-]+)/)[1];
        this.device_id = this.ck[0].match(/device_id=([\w-]+)/)[1];
        this.aid = this.ck[0].match(/aid=([\w-]+)/)[1];
        ...
        this.version_name = '26.7.0';
        this.app_name = 'douyin_lite';
        this.version_code = '260700';
        this.update_version_code = '26709926';
...
// ============================================执行项目============================================ \\
        ...
        return new Promise((resolve) => {
            let url = {
                ...
                headers: {
                    ...
                    'x-vc-bdturing-sdk-version': this.xvcbdturingsdkversion,
                    'passport-sdk-version': this.passportsdkversion,
                    'X-Khronos': times,
                    ...
                    if (error) {
                        ...
                        if (result.err_no == 0) {
                            if(result.data.task_list && Array.isArray(result.data.task_list)) {
                                for(let i=0; i < result.data.task_list.length; i++) {
                                    let taskItem = result.data.task_list[i];
                                    if (taskItem.task_id == "1007") {
                                        let tasklist = JSON.parse(taskItem.status_extra);
                                        if (taskItem.completed == true) {
                                            DoubleLog(`\n ❌ 【${this.index}】观看视频: 今日观看视频已达上限，请明日再来`)
                                        } else {
                                            DoubleLog(`\n ✅ 【${this.index}】视频统计: 已赚 ${tasklist.daily_score_amount} 金,观看时长 ${$.time('mm:ss', tasklist.daily_read_duration * 1000)}分钟`);
                                            for (let i = 1; i < numvodie; i++) {
                                                await $.wait(1000);
                                                if (this.recordstop == true) {
                                                    break;
                                                } else {
                                                    await this.result_rain_video(i,2 * 1000);
                                                }
                                            }
                                        }
                                    }
                                ...
                                DoubleLog(`\n ❌ 【${this.index}】观看视频: 此账号没有该任务，不做执行操作。`)
                            }
                        } else {
    ...
    async result_rain_video(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
    ...
    async shopping_gold(timeout = 2000) {
        return new Promise((resolve) => {
            let url = {
                url: `${hostname}/luckycat/aweme/v1/task/done/shopping_gold?mode=done&iid=${this.iid}&device_id=${this.device_id}&ac=wifi&channel=independent_gray_26709926&aid=${this.aid}&app_name=douyin_lite&version_code=${this.version_code}&version_name=${this.version_name}&device_platform=android&os=android&ssmix=a&device_type=${this.device_type}&device_brand=${this.device_brand}&language=zh&os_api=33&os_version=13&manifest_version_code=260701&resolution=1080*2166&dpi=440&update_version_code=${this.update_version_code}&_rticket=${timestamp}&gold_container=0&last_deeplink_update_version_code=0&cpu_support64=true&host_abi=arm64-v8a&is_guest_mode=0&app_type=normal&minor_status=0&appTheme=light&need_personal_recommend=1&is_android_pad=0&is_android_fold=0&ts=${times}&md=0`,
                headers: {
                    Host: host,
                    'Connection': 'Keep-Alive',
            ...
            $.post(url, async (error, response, data) => {
                try {
                    if (error) {
                        ...
                        let result = JSON.parse(data);
                        if (result.err_no == 0) {
                            if (result.data.redeem_text == null) {
                                DoubleLog(`\n ✅ 【${this.index}】逛街奖励: 获得奖励 ${result.data.amount} 金币`);
                            } else {
                                DoubleLog(`\n ❌ 【${this.index}】逛街奖励: 无法领取逛街奖励,过一段时间再来`);
                            }
                        } else {
