//name: 电信营业厅
//cron: 0 13 * * *
//变量名：chinaTelecomAccount 格式：手机号#六位密码
...
(function (_0x1398c0) {
  ...
  // ==================== Bark 推送配置 ====================
  if (typeof process.env.PUSH_SWITCH === 'undefined') {
    process.env.PUSH_SWITCH = "1"; // 推送开关，1开启，0关闭
  }
  const CT_BARK_GROUP = "中国电信";    // 自定义分组
  const CT_BARK_ICON  = "https://gitee.com/hlt1995/BARK_ICON/raw/main/ChinaTelecom.png";    // 自定义图标
  // =======================================================
  ...
  class _0x173a90 extends _0x2ecb9a {
    ...
    async ["generateNotifyContent"]() {
      let notifyContent = `📱 账号：【${this.name.slice(0, 3)}****${this.name.slice(-4)}】\n`;
      
      if (this.signDays) {
        notifyContent += `✨️ 签到成功，已签到【${this.signDays}】天\n`;
      } else {
        notifyContent += `✨️ 签到状态：未知\n`;
      }
      
      if (this.level) {
        notifyContent += `🐕️ 宠物等级[Lv.${this.level}], 升级进度: ${this.growthValue || 0}/${this.fullGrowthCoinValue || 0}\n`;
      }
      
      if (this.coin !== undefined) {
        const todayEarned = this.todayEarned || 0;
        const earnedSymbol = todayEarned > 0 ? `（+${todayEarned}）` : todayEarned < 0 ? `（${todayEarned}）` : '';
        notifyContent += `💰️ 金豆余额: 【${this.coin}】${earnedSymbol}\n`;
      }
      
      if (!_0x1a907b.formattedNotifyArray) {
        _0x1a907b.formattedNotifyArray = [];
      }
      _0x1a907b.formattedNotifyArray.push(notifyContent);
      ...
      try {
        {
          ...
          if (_0x590219 == 0) {
            ...
            const cache = _0x4f8ac1[this.name] || {};
            const yesterday = cache.coin || 0;
            this.todayEarned = this.coin - yesterday;
          ...
          {
            {
              ...
              if (_0x5beae4 == 0) {
                ...
                this.signDays = _0x972416;
                this.log("到签已".split("").reverse().join("") + _0x972416 + "天", {"notify": false});
    ...
    async ["homepage"](_0x12d2b5, _0x5240fc = {}) {
        ...
        try {
          ...
          if (_0x107703 == 0) {
            ...
            if (_0xe52288 == 0) {
              let _0x16bb06_index = 0;
              for (let _0x16bb06 of _0x49f4fc?.["atad".split("").reverse().join("")]?.["zib".split("").reverse().join("")]?.["adItems"] || []) {
                if (["0", "1"].includes(_0x16bb06?.["taskState"])) {
                  if (_0x16bb06_index > 0) {
                    let _0xwait_time = 3000 + Math.floor(Math.random() * 2000);
                    await _0x1a907b.wait(_0xwait_time);
                  }
              ...
              if (_0x3fec10 == 0) {
                {
                  ...
                  this.growthValue = _0x36ee1f?.["growthValue"];
                  this.fullGrowthCoinValue = _0x36ee1f?.["fullGrowthCoinValue"];
                  this.log("宠物等级[Lv." + _0x36ee1f?.["level"] + " :度进级升 ,]".split("").reverse().join("") + _0x36ee1f?.["growthValue"] + "/" + _0x36ee1f?.["fullGrowthCoinValue"], {"notify": false});
                  const _0x3fbf8e = {
                    "notify": true
                  };
    ...
    async ["userTask"]() {
      ...
      await this.generateNotifyContent();
  ...
  function _0xf189dd(_0x58fdd8) {
    return new class {
      ...
      async ["showmsg"]() {
        ...
        const originalBarkGroup = process.env.BARK_GROUP;
        const originalBarkIcon = process.env.BARK_ICON;
  
        process.env.BARK_GROUP = CT_BARK_GROUP;
        process.env.BARK_ICON = CT_BARK_ICON;
  
        let formattedNotify = "";
  
        if (_0x1a907b.formattedNotifyArray && _0x1a907b.formattedNotifyArray.length > 0) {
          formattedNotify += _0x1a907b.formattedNotifyArray.join('\n\n');
        } else {
          formattedNotify += this.notifyStr.join("\n");
        }
  
        await _0x5b49dd.sendNotify(this.name, formattedNotify);
  
        if (originalBarkGroup) {
          process.env.BARK_GROUP = originalBarkGroup;
        } else {
          delete process.env.BARK_GROUP;
        }
        if (originalBarkIcon) {
          process.env.BARK_ICON = originalBarkIcon;
        } else {
